...
def main():
    ...
    if not SKYENG_USERNAME or not SKYENG_PASSWORD:
        logger.error("❌ SKYENG_USERNAME или SKYENG_PASSWORD не настроены в .env")
        return
    ...
    try:
        ...
        login_page_response = session.get(login_page_url, timeout=REQUEST_TIMEOUT)
        ...
        print("🔐 Выполняю вход...")
        login_data = {
            "username": SKYENG_USERNAME,
            "password": SKYENG_PASSWORD,
        ...
        if 'login_response' in locals():
            error_data["response_status"] = login_response.status_code
            error_data["response_text"] = login_response.text[:500]
